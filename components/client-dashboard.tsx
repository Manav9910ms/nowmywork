...
export default function ClientDashboard({ user }: Props) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      await createJob({ clientId: user.uid, title: title.trim(), description: description.trim(), budget: Math.round(amount), durationDays: days, skills: skillList, techStack: stackList, priority });
      setTitle('');
      setDescription('');
      setBudget('');
      setDurationDays('7');
  ...
  return (
    <section className={styles.shell}>
      ...
      {showForm && (
        <form className={styles.formCard} onSubmit={submit}>
          <div className={styles.formIntro}>
            <div><div className="eyebrow muted">NEW PROJECT</div><h2>Tell us what needs to get done.</h2></div>
            ...
            <div className={styles.priorityGrid}>
              {(Object.keys(priorityLabels) as JobPriority[]).map((key) => (
                <button key={key} type="button" className={priority === key ? styles.prioritySelected : styles.priority} onClick={() => setPriority(key)}>
                  <strong>{priorityLabels[key].title}</strong><span>{priorityLabels[key].text}</span>
                </button>
              ))}
            </div>
          </fieldset>
      ...
      <div className={styles.jobsHeader}>
        <div><div className="eyebrow muted">YOUR JOBS</div><h2>Work you’ve posted.</h2></div>
        <span>{jobs.length} {jobs.length === 1 ? 'project' : 'projects'}</span>
      </div>

      {loadingJobs ? <div className={styles.empty}>Loading your work…</div> : jobs.length === 0 ? (
        <div className={styles.empty}><strong>Your first project starts here.</strong><p>Post a job and NowMyWork will have the information it needs to start matching.</p><button className="primary-btn" onClick={() => setShowForm(true)}>Post your first job →</button></div>
      ) : (
          ...
          {jobs.map((job) => (
            <article key={job.id} className={styles.jobCard}>
              <div className={styles.jobMain}>
                ...
                <h3>{job.title}</h3>
                <p>{job.description}</p>
                <div className={styles.chips}>{job.skills.slice(0, 4).map((skill) => <span key={skill}>{skill}</span>)}{job.skills.length > 4 && <span>+{job.skills.length - 4}</span>}</div>
              </div>
              ...
              <div className={styles.matchWrap}>
                ...
                {['ASSIGNED', 'IN_PROGRESS', 'COMPLETED'].includes(job.status) && <Link href={`/project/${job.id}`} className="secondary-btn">Open project →</Link>}
              </div>
            </article>
          ))}
        </div>
