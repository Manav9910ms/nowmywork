...
function isoDate(value: unknown) {
  const date = value && typeof (value as { toDate?: unknown }).toDate === 'function' ? (value as { toDate: () => Date }).toDate() : null;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const snapshot = await adminDb().collection('offers').where('freelancerId', '==', user.uid).limit(100).get();
    const offers = snapshot.docs.map((item) => {
      ...
      const expired = data.status === 'PENDING' && expiresAt !== null && Date.parse(expiresAt) <= Date.now();
      return { id: item.id, ...data, status: expired ? 'EXPIRED' : data.status, createdAt, updatedAt, expiresAt };
  ...
  try {
    ...
    if (body.action === 'DECLINED') {
      await db.runTransaction(async (transaction) => {
        ...
        if (data.status !== 'PENDING') throw new Error('OFFER_UNAVAILABLE');
        if (data.expiresAt?.toMillis && data.expiresAt.toMillis() <= Date.now()) throw new Error('OFFER_EXPIRED');
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      const jobRef = db.collection('jobs').doc(String(offerData.jobId));
      const job = await transaction.get(jobRef);
      if (!job.exists) throw new Error('JOB_NOT_FOUND');
      const jobData = job.data()!;
      if (jobData.assignedToId || String(jobData.status) === 'ASSIGNED' || String(jobData.status) === 'IN_PROGRESS' || String(jobData.status) === 'SUBMITTED' || String(jobData.status) === 'COMPLETED') throw new Error('JOB_ASSIGNED');
      if (!['OPEN', 'OFFERED'].includes(String(jobData.status))) throw new Error('JOB_UNAVAILABLE');

      const freelancerRef = db.collection('freelancers').doc(user.uid);
      const freelancer = await transaction.get(freelancerRef);
      const availability = String(freelancer.data()?.availability ?? 'AVAILABLE');
      if (['BUSY', 'UNAVAILABLE'].includes(availability)) throw new Error('FREELANCER_BUSY');

      transaction.update(jobRef, { status: 'ASSIGNED', assignedToId: user.uid, assignedToName: String(offerData.freelancerName ?? user.token.name ?? 'Freelancer'), assignedAt: now, updatedAt: now });
      transaction.update(offerRef, { status: 'ACCEPTED', respondedAt: now, updatedAt: now });
      transaction.set(freelancerRef, { availability: 'BUSY', updatedAt: now }, { merge: true });
      transaction.set(db.collection('paymentParties').doc(`${jobRef.id}_${user.uid}`), { jobId: jobRef.id, uid: user.uid, role: 'FREELANCER', email: user.email, phone: user.token.phone_number ?? null, updatedAt: now }, { merge: true });
    ...
    const pending = await db.collection('offers').where('jobId', '==', result.jobId).where('status', '==', 'PENDING').limit(50).get();
    if (!pending.empty) {
    ...
    if (message === 'JOB_UNAVAILABLE') return NextResponse.json({ error: 'This project is no longer available for acceptance.' }, { status: 409 });
    if (message === 'FREELANCER_BUSY') return NextResponse.json({ error: 'Your profile is marked busy, so this project cannot be accepted.' }, { status: 409 });
