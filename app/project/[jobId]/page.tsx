    ...
    <main className={styles.page}>
      ...
      <section className={styles.shell}>
        ...
        <div className={styles.heroRow}>
          <div><div className="eyebrow muted">PROJECT WORKSPACE</div><div className={styles.statusPill}>{job.status}</div><h1>{job.title}</h1><p>{statusCopy[job.status]}</p></div>
        ...
        <div className={styles.grid}>
          <section className={styles.mainCard}><div className="eyebrow muted">PROJECT DETAILS</div><h2>What needs to get done.</h2><p className={styles.description}>{job.description}</p><div className={styles.chips}>{job.skills.map((skill) => <span key={skill}>{skill}</span>)}</div>{job.techStack.length > 0 && <div className={styles.tech}><strong>Tech stack</strong><span>{job.techStack.join(' · ')}</span></div>}</section>
          <aside className={styles.sideCard}><div className="eyebrow muted">PROJECT TERMS</div><div className={styles.term}><span>Budget</span><strong>₹{job.budget.toLocaleString('en-IN')}</strong></div><div className={styles.term}><span>Duration</span><strong>{job.durationDays} days</strong></div><div className={styles.term}><span>Posted</span><strong>{formatDate(job.createdAt)}</strong></div><div className={styles.term}><span>Target date</span><strong>{getDeadline(job)}</strong></div><div className={styles.term}><span>Priority</span><strong>{job.priority === 'QUALITY' ? 'Quality First' : job.priority === 'SPEED_BUDGET' ? 'Budget / Speed' : 'Balanced'}</strong></div></aside>
        </div>

        {role && (role === 'CLIENT' || role === 'FREELANCER') && job.status === 'ASSIGNED' && <PaymentGate job={job} role={role} />}
