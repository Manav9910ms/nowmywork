...
function getDeadline(job: JobRecord) { const created = job.createdAt?.toDate(); if (!created) return 'Calculated from project start'; const deadline = new Date(created); deadline.setDate(deadline.getDate() + job.durationDays); return new Intl.DateTimeFormat('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }).format(deadline); }
...
export default function ProjectPage() {
  ...
  const [user, setUser] = useState<User | null>(null); const [role, setRole] = useState<AccountRole | null>(null); const [job, setJob] = useState<JobRecord | null>(null); const [freelancer, setFreelancer] = useState<FreelancerSummary | null>(null); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [message, setMessage] = useState('');

  useEffect(() => onAuthStateChanged(auth, async (currentUser) => {
    setUser(currentUser); if (!currentUser) { setRole(null); setJob(null); setLoading(false); return; }
    try {
      const account = await getDoc(doc(db, 'users', currentUser.uid)); setRole((account.data()?.role as AccountRole | undefined) ?? 'CLIENT');
      const loadedJob = await getJob(jobId); if (!loadedJob) { setError('This project could not be found.'); setLoading(false); return; }
      ...
      setJob(loadedJob);
      if (loadedJob.assignedToId) { const profileSnapshot = await getDoc(doc(db, 'freelancers', loadedJob.assignedToId)); if (profileSnapshot.exists()) { const data = profileSnapshot.data() as DocumentData; setFreelancer({ displayName: typeof data.displayName === 'string' ? data.displayName : undefined, skills: Array.isArray(data.skills) ? data.skills : [] }); } }
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'We could not load this project.'); } finally { setLoading(false); }
  }), [jobId]);
  ...
  async function changeStatus(nextStatus: JobStatus) {
    ...
    try {
      ...
      setJob((current) => current ? { ...current, status: data.status ?? nextStatus } : current); setMessage(nextStatus === 'IN_PROGRESS' ? 'Project started. The work is now in progress.' : nextStatus === 'SUBMITTED' ? 'Work submitted for client review.' : 'Project marked complete.');
